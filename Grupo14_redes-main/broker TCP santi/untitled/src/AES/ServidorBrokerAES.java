    ...
    private void manipularCliente(Socket socketCliente) {
        try {
            ...
            while ((linea = lector.readLine()) != null) {
                if (primera) {
                    clavesRSAClientes.put(escritor, Metodos.base64ClavePublica(linea));//recibe la clave publica del cliente y la pone en el hashmap
                    escritor.println(Metodos.clavePublicaBase64(publicKey));//envia la clave publica
                    primera = false;
                    clavesAESClientes.put(escritor,Metodos.generarClaveAES());//pone en el hashmap la clave aes del cliente
                    escritor.println(Metodos.cifrarClaveAES(clavesAESClientes.get(escritor),privateKey,clavesRSAClientes.get(escritor)));//envia la clave aes al cliente
                    continue;//cierra while
                }
