...
//decifra con asimetrico
    ...
    public static String hashear(String mensaje){
        try {
            // Crea una instancia del objeto MessageDigest con el algoritmo SHA-256
            MessageDigest digest = MessageDigest.getInstance("SHA-256");

            // Obtiene los bytes del string de entrada
            byte[] encodedHash = digest.digest(mensaje.getBytes(StandardCharsets.UTF_8));

            // Convirte los bytes de hash en una representación hexadecimal
...
//clavesRSAClientes.get(escritor)  // recibo la clave publica del cliente
//clavesAESClientes.get(escritor)  // recibo la claveAES simetrica